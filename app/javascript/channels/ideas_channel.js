...
const buildVotingDonePage = (ideas, anonymous) => {
  ...
  if (ideas.length === 0) {
    document.getElementById("ideasVotingDone").innerHTML = ""
  }
  ...
  for (let i = 0; i < ideas.length; i++) {
    ...
    htmlIdeaAndIdeaBuilds += `<div class="flex flex-col lg:mr-4"><div class="my-shadow my-8 lg:my-4 bg-white group"><div class="idea-card lg:h-64 lg:w-64 w-full lg:px-4 px-10 lg:pt-6 lg:pb-1 pt-14 pb-8 flex flex-col items-start italic font-bold bg-post-it-yellowy cursor-default"><div class="flex flex-row justify-between w-full"><h2 class="mb-4 lg:m-0 text-blurple text-5xl lg:text-xl">#${ideas[i].number}</h2><h2 class="mb-4 lg:m-0 text-blurple text-5xl lg:text-xl">${ideas[i].votes} ${ideas[i].vote_in_plural_or_singular}</h2></div><div class="flex flex-col justify-between h-full"><p class="font-bold mb-4 lg:m-0 text-4xl lg:text-base leading-loose lg:leading-normal break-words w-full">${ideas[i].text}</p><p class="font-extralight lg:m-0 text-2xl lg:text-sm leading-loose lg:leading-normal break-words w-full">${anonymous ? "" : ideas[i].author}</p></div></div>${htmlIdeaBuilds}<div class="hidden group-hover:flex"><div class="flex flex-col w-full"><form class="button_to ${ (ideas.length < 2) ? 'hidden' : ''}" method="post" action="/ideas/${ideas[i].id}/show_choose_idea_to_merge_into" data-remote="true"><button class="lg:text-base text-5xl hover:bg-blurple hover:text-white font-semibold lg:py-2 lg:px-4 px-10 py-10 w-full text-blurple bg-yellowy justify-between flex flex-row" type="submit"><p>Merge</p><span class="material-icons lg:text-2xl text-6xl">merge_type</span></button><input type="hidden" name="authenticity_token" value="${auth_token}" autocomplete="off"></form>  <form class="deleteIdeaButton" method="post" action="/ideas/${ideas[i].id}"><input type="hidden" name="_method" value="delete" autocomplete="off"><button class="lg:text-base text-5xl hover:bg-blurple hover:text-white font-semibold lg:py-2 lg:px-4 px-10 py-10 w-full text-blurple bg-post-it-yellowy justify-between flex flex-row" type="submit"><p>Delete</p><span class="material-icons lg:text-2xl text-6xl">delete</span></button><input type="hidden" name="authenticity_token" value="${auth_token}" autocomplete="off"></form></div></div></div></div></div>`
...
const buildIdeationPage = (ideas, anonymous) => {
  ...
  for (let i = 0; i < ideas.length; i++) {
    ...
    htmlIdeaAndIdeaBuilds += `<div class="flex flex-col lg:mb-12 mb-28"><div id="group-hover-div-${ideas[i].id}" class="group my-shadow mr-8 my-8 lg:my-4 bg-white"><div class="relative relativeClass"><div class="select-none idea-card lg:h-64 lg:w-64 w-full lg:px-4 px-10 lg:pt-6 lg:pb-1 pt-14 pb-8 flex flex-col items-start justify-between italic bg-post-it-yellowy cursor-default"><h2 class="font-bold mb-4 lg:m-0 text-blurple text-5xl lg:text-xl">#${ideas[i].number}</h2><div class="flex flex-col justify-between h-full"><p class="font-bold mb-4 lg:m-0 text-4xl lg:text-base leading-loose lg:leading-normal break-words w-full">${ideas[i].text}</p><p class="font-extralight lg:m-0 text-2xl lg:text-sm leading-loose lg:leading-normal break-words w-full">${anonymous ? "" : ideas[i].author}</p></div></div>${htmlIdeaBuilds}<div id="idea-sticky-container-${ideas[i].id}" class="absolute absoluteClass w-full z-50 bg-white my-shadow"><div id="idea-sticky-${ideas[i].id}" class="hidden group-hover:flex flex-row items-center lg:h-14 h-32"><div class="flex flex-row w-full leading-none justify-center text-white h-32 lg:h-14"><form class="w-full" method="post" action="/ideas/${ideas[i].id}/show_idea_build_form" data-remote="true"><button class="h-full w-full" type="submit"><p class="bg-blurple hover:bg-post-it-yellowy hover:text-blurple hover:border-2 hover:border-blurple hover:border-solid h-full flex flex-col justify-center lg:text-base text-5xl uppercase font-bold">Build on idea</p></button><input type="hidden" name="authenticity_token" value="${auth_token}" autocomplete="off"></form><form class="toggleIdeaMenuButton" method="post" action="/ideas/${ideas[i].id}/toggle_idea_menu" data-remote="true"><button class="h-full w-full bg-white hover:bg-post-it-yellowy" id="more-options-${ideas[i].id}" type="submit"><span class="lg:w-8 w-32 material-icons hover:border-2 hover:border-blurple hover:border-solid h-full text-blurple flex flex-col justify-center lg:text-2xl text-6xl">more_vert</span></button><input type="hidden" name="authenticity_token" value="${auth_token}" autocomplete="off"></form></div></div><div id="add-idea-build-form-idea-${ideas[i].id}"></div></div><div id="idea-menu-idea-${ideas[i].id}"></div></div></div></div>`
